...
class JsonApiExporter:
            ...
            return {
                ...
                "metadata": {
                    ...
                    "pagination": {
                        ...
                        "next_offset": (offset or 0) + limit if (offset or 0) + len(metrics_data) < total_count else None,
                        "previous_offset": max(0, (offset or 0) - limit) if (offset or 0) > 0 else None
                    }
                }
            }
        ...
        try:
            ...
            summary_data = {
                "time_period": {
                    ...
                    "duration_hours": (summary.end_time - summary.start_time).total_seconds() / 3600
                },
                "requests": {
                    ...
                    "success_rate": 1.0 - summary.error_rate if summary.total_requests > 0 else 0.0
                },
                "performance": {
                ...
                "usage": {
                    ...
                    "requests_per_hour": (
                        summary.total_requests / ((summary.end_time - summary.start_time).total_seconds() / 3600)
                        if (summary.end_time - summary.start_time).total_seconds() > 0 else 0.0
                    )
                },
    ...
    def _metric_to_dict(self, metric: MetricRecord) -> Dict[str, Any]:
        ...
        from ..models import RequestMetric, ResponseMetric, ErrorMetric, CostMetric, LatencyMetric, UsageMetric
