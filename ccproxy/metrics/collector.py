...
class MetricsCollector:
    ...
    def __init__(
        ...
        self._metrics_by_type: Dict[MetricType, int] = {metric_type: 0 for metric_type in MetricType}
    ...
    async def collect_response(
        ...
        **kwargs: Any
        ...
        if (input_tokens is not None or output_tokens is not None) and request_id in self._active_requests:
            request_metric = self._active_requests[request_id]
    ...
    @asynccontextmanager
    async def request_context(self, request_id: str, **kwargs: Any) -> AsyncIterator[RequestMetric]:
        """
        ...
        Yields:
            RequestMetric object
        """
        request_metric = await self.collect_request_start(request_id=request_id, **kwargs)
    ...
    def _calculate_summary(
        ...
        summary.successful_requests = len([r for r in responses if 200 <= r.status_code < 300])
