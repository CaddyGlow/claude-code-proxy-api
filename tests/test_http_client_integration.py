...
from ccproxy.exceptions import AuthenticationError, RateLimitError, ServiceUnavailableError
...
class TestOAuthFlowWithInstrumentedClient:
    ...
    @pytest.mark.asyncio
    async def test_oauth_refresh_token(self, http_client, mock_settings, expired_credentials):
    ...
    @pytest.mark.asyncio
    async def test_oauth_profile_fetching(self, http_client, mock_settings, valid_credentials):
...
class TestCredentialsManagerIntegration:
    ...
    async def test_concurrent_refresh_handling(
        ...
        with patch.object(http_client._client, "request", side_effect=mock_request):
            ...
            tasks = [
                credentials_manager.get_valid_credentials()
                for _ in range(5)
            ]
...
class TestMiddlewareChainBehavior:
    ...
    @pytest.mark.asyncio
    async def test_retry_middleware_with_metrics(self, http_client_config, metrics_storage):
        """Test retry middleware with metrics collection."""
        # Create client with metrics
        client = create_http_client(config=http_client_config, metrics_storage=metrics_storage)
...
class TestErrorScenarios:
    ...
    async def test_http_error_codes(self, http_client):
        ...
        for code in error_codes:
            ...
            with patch.object(http_client._client, "request", return_value=mock_response):
                response = await http_client.request("GET", f"https://api.example.com/error/{code}")
...
class TestPerformanceComparison:
    ...
    async def test_instrumented_client_overhead(self):
        ...
        with patch.object(instrumented_client._client, "request", return_value=mock_response):
    ...
    async def test_streaming_performance(self, http_client):
        ...
        with patch.object(http_client._client, "stream") as mock_stream_method:
            ...
            async with http_client.stream("GET", "https://api.example.com/large") as response:
