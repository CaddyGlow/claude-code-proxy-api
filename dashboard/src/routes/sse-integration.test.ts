...
import { flushPromises, setupFakeTimers, cleanupFakeTimers } from '../test-utils/helpers/test-helpers';
import DashboardPage from './+page.svelte';

// Mock the metrics API
vi.mock('$lib/services/metrics-api', () => ({
	metricsApi: {
		getAnalytics: vi.fn(),
...
describe('SSE Integration Tests', () => {
	...
	beforeEach(() => {
		...
		// Mock the metrics API
		vi.mocked(metricsApi.getAnalytics).mockResolvedValue(mockAnalyticsData);

		// Setup mock SSE connection
		mockSSEConnection = createSSEMock('http://localhost:8000/metrics/stream');
		vi.mocked(metricsApi.createSSEConnection).mockReturnValue(mockSSEConnection as any);
	});
	...
	describe('Dashboard SSE Integration', () => {
		...
		it('should handle SSE connection events and show notifications', async () => {
			...
			// Assert
			expect(screen.getByText('Connected: Client connected successfully')).toBeInTheDocument();
		});
		...
		it('should handle SSE disconnect events and show notifications', async () => {
			...
			// Assert
			expect(screen.getByText('Disconnected: Server shutting down')).toBeInTheDocument();
		});
		...
		it('should handle malformed SSE messages gracefully', async () => {
			...
			expect(consoleSpy).toHaveBeenCalledWith('Failed to parse stream event:', expect.any(Error));
	...
	describe('Connection Loss and Recovery', () => {
		...
		it('should handle SSE setup errors gracefully', async () => {
			// Arrange
			const consoleSpy = vi.spyOn(console, 'error').mockImplementation(() => {});
			vi.mocked(metricsApi.createSSEConnection).mockImplementation(() => {
			...
			expect(consoleSpy).toHaveBeenCalledWith('Failed to setup SSE:', expect.any(Error));
	...
	describe('Real-time Data Updates', () => {
		...
		it('should auto-remove notifications after timeout', async () => {
			...
			expect(screen.queryByText('Connected: Test notification')).not.toBeInTheDocument();
		});
	...
	describe('Filter Integration with SSE', () => {
		it('should maintain SSE connection when filters change', async () => {
			...
			const initialCallCount = vi.mocked(metricsApi.createSSEConnection).mock.calls.length;
...
timeRangeSelect.dispatchEvent(changeEvent);
			...
			expect(vi.mocked(metricsApi.createSSEConnection).mock.calls.length).toBe(initialCallCount);
		});
...
Object.defineProperty(serviceChangeEvent, 'target', { value: { value: 'anthropic' } });
...
Object.defineProperty(modelChangeEvent, 'target', { value: { value: 'claude-3-sonnet' } });
