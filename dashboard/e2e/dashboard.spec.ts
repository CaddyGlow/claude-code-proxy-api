...
test.describe("Dashboard E2E Tests - Critical P1 Flows", () => {
	...
	test("should load and display metric cards with correct data", async ({
		page,
	}) => {
		// Arrange
		...
		await expect(
			page.getByRole("heading", { name: /claude code proxy/i }),
		).toBeVisible();
		...
		const totalRequestsValue =
			await dashboardPage.getMetricCardValue("total-requests");
		expect(totalRequestsValue).toBe("1250");

		const successRateValue =
			await dashboardPage.getMetricCardValue("success-rate");
		expect(successRateValue).toBe("96.0%");

		const avgResponseTimeValue =
			await dashboardPage.getMetricCardValue("avg-response-time");
	...
	test("should update data when time range buttons are clicked", async ({
		page,
	}) => {
		// Arrange
		await dashboardPage.goto();
	...
	test("should handle real-time SSE updates correctly", async ({ page }) => {
		...
		const closeButton = page
			.locator('.fixed.top-4.right-4 button[aria-label="Close notification"]')
			.first();
		if (await closeButton.isVisible()) {
			await closeButton.click();
			await page.waitForTimeout(500);
			const notificationCount = await page
				.locator(".fixed.top-4.right-4 .bg-blue-600")
				.count();
			expect(notificationCount).toBeLessThan(1);
		}
	});

	test("should display appropriate error states when API fails", async ({
		page,
	}) => {
	...
	test("should handle loading states and transitions smoothly", async ({
		page,
	}) => {
	...
	test("should handle URL parameter persistence correctly", async ({
		page,
	}) => {
	...
	test("should handle various SSE event types appropriately", async ({
		page,
	}) => {
		// Arrange
		const mixedEvents = [
		...
		const notifications = page.locator(
			".fixed.top-4.right-4 .bg-blue-600, .fixed.top-4.right-4 .bg-red-600",
		);
